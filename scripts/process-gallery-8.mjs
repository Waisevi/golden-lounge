...
async function updateGalleryEighthImage(imagePath) {
    console.log(`🔄 Updating eighth image in gallery (order = 8)...`);
    ...
    // Find the item with order = 8 (eighth item)
    const eighthItem = galleryItems.find(item => item.order === 8) || galleryItems[7];
    
    if (!eighthItem) {
        console.error(`❌ No eighth gallery item found`);
        throw new Error('No eighth gallery item found');
    }
    
    // Update the eighth item
    ...
    console.log(`✅ Updated gallery item ${eighthItem.id} (order: ${eighthItem.order}) with new image: ${imagePath}`);
}

async function main() {
    try {
        const publicDir = path.join(process.cwd(), 'public');
        const inputFile = path.join(publicDir, 'IMG_4407.JPG');
        const outputFile = path.join(publicDir, 'gallery-8.webp');
        ...
        const storagePath = 'images/gallery/8.webp';
        await uploadToStorage(outputFile, storagePath);
        
        // Step 3: Update gallery in database (8th image, order = 8)
        await updateGalleryEighthImage(storagePath);
