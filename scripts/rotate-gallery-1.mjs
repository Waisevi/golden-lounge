...
async function processImage() {
    try {
        const inputPath = path.join(__dirname, '..', 'public', 'images', 'gallery', '1.webp');
        const tempPath = path.join(__dirname, '..', 'public', 'images', 'gallery', '1_temp.webp');
        const outputPath = path.join(__dirname, '..', 'public', 'images', 'gallery', '1.webp');
        const storagePath = 'images/gallery/1.webp';

        if (!fs.existsSync(inputPath)) {
            console.error('❌ Файл 1.webp не найден');
            process.exit(1);
        }

        console.log('📸 Обрабатываю изображение галереи...');
        ...
        console.log('🔄 Поворачиваю на 90° против часовой стрелки...');

        // Сжимаем до 1 МБ с поворотом
        let quality = 70;
        let outputSize = 0;
        let attempts = 0;
        ...
        while (attempts < maxAttempts) {
            ...
            await sharp(inputPath)
                .rotate(-90) // Поворот на 90° против часовой стрелки
                .resize(width, height, { 
                    fit: 'inside',
        ...
        console.log(`\n✨ Готово! Изображение галереи повернуто и сжато до 1MB.`);
